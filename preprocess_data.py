...
def preprocess_pipeline(file_path, output_dir):
    ...
    for col in cat_cols:
        ...
        # Transform Val and Test (Handle unknowns)
        unknown_label = -1
        le_col_classes = set(le_col.classes_)
        X_val[col] = X_val[col].apply(lambda x: le_col.transform([x])[0] if x in le_col_classes else unknown_label)
        X_test[col] = X_test[col].apply(lambda x: le_col.transform([x])[0] if x in le_col_classes else unknown_label)
    ...
    # 8. Save
    print("Saving processed data (Pickle)...")
    # Train
    ...
    print(f"Saved train_data.pkl ({train_df.shape}), val_data.pkl ({val_df.shape}), test_data.pkl ({test_df.shape})")

if __name__ == "__main__":
    FILE_PATH = "c:/Users/mizal/.gemini/antigravity/scratch/iot_project_2/RT_IOT2022.csv"
    OUTPUT_DIR = "c:/Users/mizal/.gemini/antigravity/scratch/iot_project_2/processed_data"
    
    preprocess_pipeline(FILE_PATH, OUTPUT_DIR)
